...
use_asr_preprocessor=true  # see espnet2/tasks/asr.py line 320 for options: can include rir/noise/...
...
decode_ngram= # Use pre-built ngram file for decoding

# [Task dependent] Set the datadir name created by local/data.sh
train_set=train_s       # Name of training set.
valid_set=valid_s       # Name of validation set used for monitoring/tuning network training.
test_sets="dev_s valid_s"       # Names of test sets. Multiple items (e.g., both dev and eval sets) can be specified.
...
asr_task=cgn  # cgn (verbatim) or subtitles, which data to prepare
...
Options:
    ...
    --use_maskctc         # Whether to use maskctc decoding (default="${use_maskctc}").
...
if [ -z "${lm_stats_dir}" ]; then
    if [ "${lang}" != noinfo ]; then
        lm_stats_dir="${lm_dir}/lm_stats_${lang}_${lm_token_type}"
    else
        lm_stats_dir="${lm_dir}/lm_stats_${lm_token_type}"
    fi
    if [ "${lm_token_type}" = bpe ]; then
        lm_stats_dir+="${nbpe}"
    fi
...
if ! "${skip_data_prep}"; then
    ...
    if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
        ...
        if [ -n "${speed_perturb_factors}" ]; then
           for spkdir in data/finetune/s*; do
             SPK=$(basename $spkdir)
             _dirs=""

             log "Stage 2: Speed perturbation: data/finetune/${SPK}/train_${SPK} -> data/finetune/${SPK}/train_${SPK}_sp"
             for factor in ${speed_perturb_factors}; do
                 if [[ $(bc <<<"${factor} != 1.0") == 1 ]]; then
                     scripts/utils/perturb_data_dir_speed.sh "${factor}" "data/finetune/${SPK}/train_${SPK}" "data/finetune/${SPK}/train_${SPK}_sp${factor}"
                     _dirs+="data/finetune/${SPK}/train_${SPK}_sp${factor} "
                 else
                     # If speed factor is 1, same as the original
                     _dirs+="data/finetune/${SPK}/train_${SPK} "
                 fi
             done
             utils/combine_data.sh --extra_files "text.lc.subtitle text.lc.verbatim" "data/finetune/${SPK}/train_${SPK}_sp" ${_dirs}
           done
        else
        ...
        for factor in ${speed_perturb_factors}; do
            if [[ $(bc <<<"${factor} != 1.0") == 1 ]]; then
                 scripts/utils/perturb_data_dir_speed.sh "${factor}" "data/finetune/train_all" "data/finetune/train_all_sp${factor}"
                 _dirs+="data/finetune/train_all_sp${factor} "
            else
                 # If speed factor is 1, same as the original
                 _dirs+="data/finetune/train_all "
            fi
        done
        utils/combine_data.sh --extra_files "text.lc.subtitle text.lc.verbatim" \
          "data/finetune/train_all_sp" ${_dirs}
        fi
    fi
    ...
    if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
        if [ "${feats_type}" = raw ]; then
            ...
            if false; then
            for dset in train_all train_all_sp; do
                cp -r data/finetune/${dset}/ "${data_feats}/finetune/"
                _nj=$(min "${nj}" "$(<"${data_feats}/finetune/${dset}/utt2spk" wc -l)")
                . ./path-kaldi.sh
                utils/fix_data_dir.sh --utt_extra_files "text.lc.verbatim text.lc.subtitle" \
                  "${data_feats}/finetune/${dset}"
                steps/make_fbank_pitch.sh --nj "${_nj}" --cmd "${train_cmd}" \
                  "${data_feats}/finetune/${dset}"
                . ./path.sh
                ...
                scripts/feats/feat_to_shape.sh --nj "${_nj}" --cmd "${train_cmd}" \
                  "${data_feats}/finetune/${dset}/feats.scp" \
                  "${data_feats}/finetune/${dset}/feats_shape"

                head -n 1 "${data_feats}/finetune/${dset}/feats_shape" | awk '{ print $2 }' \
                  | cut -d, -f2 > ${data_feats}/finetune/${dset}/feats_dim

                echo "${feats_type}" > "${data_feats}/finetune/${dset}/feats_type"
            done
            fi

            for spkdir in data/finetune/s*; do
                SPK=$(basename $spkdir)
                mkdir -p "${data_feats}/finetune/${SPK}"

                train_set=train_${SPK}
                train_set_sp=train_${SPK}_sp
                valid_set=valid_${SPK}
                test_set=test_${SPK}

                for dset in "${train_set}" "${valid_set}" "${test_set}"; do
                  cp -r data/finetune/${SPK}/${dset}/ "${data_feats}/finetune/${SPK}/"
                  cp "${data_feats}"/finetune/train_all/{feats.scp,feats_dim,feats_type,feats_shape,frame_shift,utt2num_frames,utt2dur} "${data_feats}/finetune/${SPK}/${dset}"

                  utils/fix_data_dir.sh --utt_extra_files "text.lc.subtitle text.lc.verbatim utt2dur utt2num_frames feats_shape" "${data_feats}/finetune/${SPK}/${dset}"
                done

                for dset in "${train_set_sp}"; do
                  cp -r data/finetune/${SPK}/"${dset}"/ "${data_feats}/finetune/${SPK}/"
                  cp "${data_feats}"/finetune/train_all_sp/{feats.scp,feats_dim,feats_type,feats_shape,frame_shift,utt2num_frames,utt2dur} "${data_feats}/finetune/${SPK}/${dset}"
    ...
    if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
        if [ "${token_type}" = bpe ]; then
            ...
            if  "${bpe_split_digits}"; then
                _opts_spm+="--split_digits=true"
            fi
...
if ! "${skip_train}"; then
    ...
    if [ ${stage} -le 10 ] && [ ${stop_stage} -ge 10 ]; then
        _asr_train_dir="${data_feats}/finetune/${train_set}"
        _asr_valid_dir="${data_feats}/finetune/${valid_set}"
        ...
        # shellcheck disable=SC2086
        OMP_NUM_THREADS=${_nj},3,1 ${train_cmd} --num_threads 4 --max-jobs-run 1 \
        JOB=1:"${_nj}" "${_logdir}"/stats.JOB.log \
            ${python} -m espnet2.bin.asr_train \
                --collect_stats true \
                --use_preprocessor "${use_asr_preprocessor}" \
                --bpemodel "${bpemodel}" \
                --token_type "${token_type}" \
...
if ! "${skip_eval}"; then
    if [ ${stage} -le 12 ] && [ ${stop_stage} -ge 12 ]; then
        ...
        for dset in ${test_sets}; do
            _data="${data_feats}/finetune/${dset}"
            ...
            OMP_NUM_THREADS=${_nj},3,1 ${_cmd} --num_threads 4 --max-jobs-run 1 --gpu "${_ngpu}" JOB=1:"${_nj}" "${_logdir}"/asr_inference.JOB.log \
