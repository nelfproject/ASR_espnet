...
overwrite_bpemodel=     # Overwrite the default bpemodel with another one, e.g. when loading a pretrained subtitle model
overwrite_token_list=  #
...
if [ ! -z "${overwrite_bpemodel}" ]; then
    bpemodel=${overwrite_bpemodel}
fi
if [ ! -z "${overwrite_token_list}" ]; then
    token_list=${overwrite_token_list}
fi
...
if ! "${skip_data_prep}"; then
    ...
    if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
        if [ "${feats_type}" = raw ]; then
            ...
            for dset in ${test_sets}; do  #helena_extended; do  #"${train_set}" "${valid_set}" ${test_sets}; do
                if [ "${dset}" = "${train_set}" ] || [ "${dset}" = "${valid_set}" ]; then
                    _suf="/org"
                else
                    _suf=""
