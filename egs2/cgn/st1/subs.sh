...
local_subs_opts= # Options for subtitle dataset and train/test generation
...
token_asr=false         # if token_joint is true, and token_asr is also true, then use the asr bpe for both tasks (for frozen asr experiments with growing amount of subtitle data)
src_case=lc.rm
...
st_train_set=       # Name of training set.
st_valid_set=       # Name of validation set used for monitoring/tuning network training.
st_test_set=       # Names of test sets. Multiple items (e.g., both dev and eval sets) can be specified.
...
feats_dir=                 # directory containing feats.scp file for extracted feats_type
token_dir_suffix=          # suffix to token dir (e.g. when only using part of data)
...
train_set=${st_train_set}
valid_set=${st_valid_set}
test_set=${st_test_set}
test_sets="${st_test_set}"
...
if [ "${feats_type}" = raw ]; then
    ...
    feats_type_suf=${feats_dir##*/}
    data_feats=${dumpdir}/extracted_${feats_type_suf}
else
    log "${help_message}"
...
[ -z "${lm_train_text}" ] && lm_train_text="${data_feats}/${train_set}/text.${src_case}.${src_lang}"  #text.${tgt_case}.${tgt_lang}
# Use the same text as ST for lm training if not specified.
[ -z "${lm_dev_text}" ] && lm_dev_text="${data_feats}/${valid_set}/text.${src_case}.${src_lang}"  #text.${tgt_case}.${tgt_lang}"
# Use the text of the 1st evaldir if lm_test is not specified
[ -z "${lm_test_text}" ] && lm_test_text="${data_feats}/${test_set}/text.${src_case}.${src_lang}"  #text.${tgt_case}.${tgt_lang}"
...
if [ -z "${token_dir_suffix}" ]; then
    token_listdir+="_${token_dir_suffix}"
fi

# The tgt bpedir is set for all cases when using bpe
if "${token_joint}"; then
    if "${token_asr}"; then
        tgt_bpedir="${token_listdir}/asr_bpe_${tgt_bpemode}${tgt_nbpe}${token_suffix}"
    else
        tgt_bpedir="${token_listdir}/joint_bpe_${tgt_bpemode}${tgt_nbpe}${token_suffix}"
    fi
else
    tgt_bpedir="${token_listdir}/subs_bpe_${tgt_bpemode}${tgt_nbpe}${token_suffix}"
fi
tgt_bpeprefix="${tgt_bpedir}"/bpe
tgt_bpemodel="${tgt_bpeprefix}".model
tgt_bpetoken_list="${tgt_bpedir}"/tokens.txt
tgt_chartoken_list="${token_listdir}"/char/subs_tokens.txt
if "${token_joint}"; then
    ...
    src_bpedir="${token_listdir}/asr_bpe_${src_bpemode}${src_nbpe}${token_suffix}"
    src_bpeprefix="${src_bpedir}"/bpe
    src_bpemodel="${src_bpeprefix}".model
    src_bpetoken_list="${src_bpedir}"/tokens.txt
    src_chartoken_list="${token_listdir}"/char/asr_tokens.txt
fi
...
if ! "${skip_data_prep}"; then
    if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
        ...
        if [ -d "data/${asr_train_set}" ] && [ -d "data/${asr_valid_set}" ] && [ -d "data/${asr_test_set}" ]; then
            log "ASR data set already exists. Skipping."
        else
            log "Preparing ASR data"
            local/prepare_data_cgn.sh ${local_data_opts}
        fi

        if [ -d "data/${subs_train_set}" ] && [ -d "data/${subs_valid_set}" ] && [ -d "data/${subs_test_set}" ]; then
            log "Subtitles data set already exists. Skipping."
        else
            log "Preparing subtitles data"
            local/prepare_data_subtitles.sh ${local_subs_opts}  # _nelf.sh
        fi

        # TODO: this script still assumes that utterances are not present in both sets
        # TODO: should make sure they are both in train or test set when we add them
    fi

    if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
        if [ -n "${speed_perturb_factors}" ]; then
            log "Stage 2: Speed perturbation: data/${asr_train_set} -> data/${asr_train_set}_sp"
            for x in ${asr_train_set}; do
                for factor in ${speed_perturb_factors}; do
                    if [[ $(bc <<<"${factor} != 1.0") == 1 ]]; then
                            scripts/utils/perturb_data_dir_speed.sh --utt_extra_files "${utt_extra_files}" \
                                 "${factor}" "data/${x}" "data/${x}_sp${factor}"
                            _dirs+="data/${x}_sp${factor} "
                    else
                        # If speed factor is 1, same as the original
                        _dirs+="data/${x} "
                    fi
                done
                utils/combine_data.sh --extra_files "${utt_extra_files}" "data/${x}_sp" ${_dirs}
                for extra_file in ${utt_extra_files}; do
                    python pyscripts/utils/remove_duplicate_keys.py data/"${x}_sp"/${extra_file} > data/"${x}_sp"/${extra_file}.tmp
                    mv data/"${x}_sp"/${extra_file}.tmp data/"${x}_sp"/${extra_file}
                done
            done
        else
    ...
    if [ -n "${speed_perturb_factors}" ]; then
        asr_train_set="${asr_train_set}_sp"
        subs_train_set="${subs_train_set}_sp"
    fi
    ...
    if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
        if [ "${feats_type}" = raw ]; then
            ...
            log "Feats type raw - Not edited for subtitles"
            exit 1;
            ...
            #for dset in "${subs_train_set}" "${subs_valid_set}"; do
            #for dset in "${asr_train_set}" "${asr_valid_set}" "${asr_test_set}" "${subs_train_set}" "${subs_valid_set}" "${subs_test_set}"; do
            for dset in "${subs_train_set}" "${subs_valid_set}"; do

                if [ "${dset}" = "${asr_train_set}" ] || [ "${dset}" = "${asr_valid_set}" ] || [ "${dset}" = "${subs_train_set}" ] || [ "${dset}" = "${subs_valid_set}" ]; then
                    _suf="/org"
                else
                    _suf=""
                fi

                if [ -f "${data_feats}${_suf}/${dset}/feats.scp" ] && [ -f "${data_feats}${_suf}/${dset}/feats_shape" ]; then
                    log "Features are already extracted for ${dset}. Skipping"
                    continue
                fi
                ...
                if [ -z "${feats_dir}" ] | [ ! -f "${feats_dir}/feats.scp" ]; then
                    . ./path-kaldi.sh
                    steps/make_fbank_pitch.sh --nj "${_nj}" --cmd "${train_cmd}" "${data_feats}${_suf}/${dset}"
                    . ./path.sh
                else
                    cp "${feats_dir}/feats.scp" "${data_feats}${_suf}/${dset}/feats.scp"
                fi
            ...
            log "Feats type extracted - Not edited for subtitles"
            exit 1;
            ...
            for dset in "${train_set}" "${valid_set}" ${test_sets}; do
                if [ "${dset}" = "${train_set}" ] || [ "${dset}" = "${valid_set}" ]; then
                    feats_file="train_feats.scp"
                    _suf="/org"
                else
                    feats_file="dev_feats.scp"
                    _suf=""
                fi
                ...
                # FEATS.SCP
                cp ${feats_dir}/${feats_file} ${data_feats}${_suf}/${dset}/feats.scp
                utils/fix_data_dir.sh ${data_feats}${_suf}/${dset}

                # CMVN.SCP
                #steps/compute_cmvn_stats.sh --fake ${data_feats}${_suf}/${dset}
                #utils/fix_data_dir.sh ${data_feats}${_suf}/${dset}
    ...
    if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
        ...
        #for dset in "${asr_train_set}" "${asr_valid_set}" "${subs_train_set}" "${subs_valid_set}"; do
        for dset in "${subs_train_set}" "${subs_valid_set}"; do
            if [ -f "${data_feats}/${dset}/feats.scp" ] && [ -f "${data_feats}/${dset}/feats_shape" ]; then
                    log "Features are already processed for ${dset}. Skipping"
                    continue
            fi
        ...
        ## Prepare subtitles and verbatim transcripts
        log "Prepping ASR targets"
        for x in "${asr_train_set}" "${asr_valid_set}" "${asr_test_set}"; do
            cp ${data_feats}/${x}/text ${data_feats}/${x}/text.${src_case}.${src_lang}
        done

        log "Prepping Subtitles targets"
        for x in "${subs_train_set}" "${subs_valid_set}" "${subs_test_set}"; do
            cp ${data_feats}/${x}/text ${data_feats}/${x}/text.${tgt_case}.${tgt_lang}
        done

        # To combine, we need to make the subs have empty asr targets and vice-versa.
        # Some data can have both asr+subs, so need to filter them out before emptying.
        log "Combining data and Preparing for multi-task learning"
        for x in "${asr_train_set}" "${asr_valid_set}" "${asr_test_set}" "${subs_train_set}" "${subs_valid_set}" "${subs_test_set}"; do
            awk '{print $1}' ${data_feats}/${x}/segments > ${data_feats}/${x}/uttlist
        done

        grep -vF -f ${data_feats}/${subs_train_set}/uttlist ${data_feats}/${asr_train_set}/text | awk '{print $1 " <unk>"}' > ${data_feats}/${asr_train_set}/text.${tgt_case}.${tgt_lang}
        grep -vF -f ${data_feats}/${subs_valid_set}/uttlist ${data_feats}/${asr_valid_set}/text | awk '{print $1 " <unk>"}' > ${data_feats}/${asr_valid_set}/text.${tgt_case}.${tgt_lang}
        grep -vF -f ${data_feats}/${subs_test_set}/uttlist ${data_feats}/${asr_test_set}/text | awk '{print $1 " <unk>"}' > ${data_feats}/${asr_test_set}/text.${tgt_case}.${tgt_lang}

        grep -vF -f ${data_feats}/${asr_train_set}/uttlist ${data_feats}/${subs_train_set}/text | awk '{print $1 " <unk>"}' > ${data_feats}/${subs_train_set}/text.${src_case}.${src_lang}
        grep -vF -f ${data_feats}/${asr_valid_set}/uttlist ${data_feats}/${subs_valid_set}/text | awk '{print $1 " <unk>"}' > ${data_feats}/${subs_valid_set}/text.${src_case}.${src_lang}
        grep -vF -f ${data_feats}/${asr_test_set}/uttlist ${data_feats}/${subs_test_set}/text | awk '{print $1 " <unk>"}' > ${data_feats}/${subs_test_set}/text.${src_case}.${src_lang}

        # Combine datasets for ST training
        utils/combine_data.sh \
            --extra-files "text.${src_case}.${src_lang} text.${tgt_case}.${tgt_lang}" \
            ${data_feats}/${st_train_set} \
            ${data_feats}/${asr_train_set} ${data_feats}/${subs_train_set}
        utils/combine_data.sh \
            --extra-files "text.${src_case}.${src_lang} text.${tgt_case}.${tgt_lang}" \
            ${data_feats}/${st_valid_set} \
            ${data_feats}/${asr_valid_set} ${data_feats}/${subs_valid_set}
        utils/combine_data.sh \
            --extra-files "text.${src_case}.${src_lang} text.${tgt_case}.${tgt_lang}" \
            ${data_feats}/${st_test_set} \
            ${data_feats}/${asr_test_set} ${data_feats}/${subs_test_set}

        _feats_type_asr ="$(<${data_feats}/${asr_train_set}/feats_type)"
        _feats_type_subs ="$(<${data_feats}/${subs_train_set}/feats_type)"
        if [ "${_feats_type_asr}" == "${feats_type_subs}" ]; then
            for dset in "${st_train_set}" "${st_valid_set}" "${st_test_set}"; do
                cp ${data_feats}/${asr_train_set}/feats_type ${data_feats}/${dset}/feats_type
            done
        else
            log "ASR and subs have different feature type. This is not supported yet"
            exit 1;
        fi

        _feats_dim_asr ="$(<${data_feats}/${asr_train_set}/feats_dim)"
        _feats_dim_subs ="$(<${data_feats}/${subs_train_set}/feats_dim)"
        if [ "${_feats_dim_asr}" == "${feats_dim_subs}" ]; then
            for dset in "${st_train_set}" "${st_valid_set}" "${st_test_set}"; do
                cp ${data_feats}/${asr_train_set}/feats_dim ${data_feats}/${dset}/feats_dim
            done
        else
            log "ASR and subs have different feature dimension. This is not supported yet"
            exit 1;
        fi
    ...
    if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
        ...
        if [ "${tgt_token_type}" = bpe ]; then
            if "${token_joint}"; then
                log "Stage 5a: Generate token_list from both ${src_bpe_train_text} and ${tgt_bpe_train_text} using BPE"
            else
                log "Stage 5a: Generate token_list from ${tgt_bpe_train_text} using BPE for tgt_lang"
            fi
            mkdir -p "${tgt_bpedir}"
            # shellcheck disable=SC2002
            # Also remove empty lines (utts without subtitle/verbatim)
            cat ${tgt_bpe_train_text} | cut -f 2- -d" " | sed '/^[[:space:]]*$/d' > "${tgt_bpedir}"/train.txt

            if "${token_joint}"; then
                cat ${src_bpe_train_text} | cut -f 2- -d" " | sed '/^[[:space:]]*$/d' >> "${tgt_bpedir}"/train.txt
            fi
        ...
        if "${token_joint}"; then
            ...
            if [ "${src_token_type}" = bpe ]; then
                ...
                mkdir -p "${src_bpedir}"
                # shellcheck disable=SC2002
                cat ${src_bpe_train_text} | cut -f 2- -d" " | sed '/^[[:space:]]*$/d' > "${src_bpedir}"/train.txt
...
if ! "${skip_train}"; then
    ...
    if [ ${stage} -le 10 ] && [ ${stop_stage} -ge 10 ]; then
        ...
        # shellcheck disable=SC2086
        ${train_cmd} JOB=1:"${_nj}" "${_logdir}"/stats.JOB.log \
            ${python} -m espnet2.bin.subtitle_train_chained \
                --collect_stats true \
    ...
    if [ ${stage} -le 11 ] && [ ${stop_stage} -ge 11 ]; then
        ...
        # TODO(jiatong): fix bpe
        # shellcheck disable=SC2086
        OMP_NUM_THREADS=${nj},3,1 ${python} -m espnet2.bin.launch \
            --cmd "${cuda_cmd} --num_threads ${nj} --name ${jobname}" \
...
if ! "${skip_eval}"; then
    if [ ${stage} -le 12 ] && [ ${stop_stage} -ge 12 ]; then
        ...
        if "${use_ngram}"; then
             if [ -n "${decode_ngram}" ]; then
                 _opts+="--ngram_file ${decode_ngram}"
             else
        ...
        for dset in ${test_sets}; do
            ...
            st_inference_tool="espnet2.bin.subtitling_inference_chained"
            ...
            OMP_NUM_THREADS=${_nj},3,1 ${_cmd} --num_threads 4 --max-jobs-run 1 --gpu "${_ngpu}" JOB=1:"${_nj}" "${_logdir}"/st_inference.JOB.log \
            ...
            if [[ -e "${_logdir}/output.1/1best_recog/subs" ]]; then
                for f in token_subs token_int_subs score_subs subs ; do
                    for i in $(seq "${_nj}"); do
                        cat "${_logdir}/output.${i}/1best_recog/${f}"
                    done | LC_ALL=C sort -k1 >"${_dir}/subs"
                done
            fi
        done
    fi
