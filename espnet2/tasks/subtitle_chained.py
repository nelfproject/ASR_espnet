...
from espnet2.asr.decoder.multi_transformer_decoder import MultiTransformerDecoder
...
from espnet2.subtitling.espnet_model_subtitle_chained import ESPnetSubtitleModelChained
...
subtitle_encoder_choices = ClassChoices(
    "subtitle_encoder",
    classes=dict(
        conformer=ConformerEncoder,
        transformer=TransformerEncoder,
...
subtitle_decoder_choices = ClassChoices(
    "subtitle_decoder",
    classes=dict(
        transformer=TransformerDecoder,
        lightweight_conv=LightweightConvolutionTransformerDecoder,
        ...
        rnn=RNNDecoder,
        mlm=MLMDecoder,
        multi_transformer=MultiTransformerDecoder,
        multi_mlm=MultiMLMDecoder,
    ),
    type_check=(AbsDecoder, AbsMultiDecoder),
    default="rnn",
    optional=True,
)


class SubtitleTask(AbsTask):
    ...
    class_choices_list = [
        ...
        decoder_choices,
        # --subtitle_encoder and --subtitle_encoder_conf
        subtitle_encoder_choices,
        # --subtitle_decoder and --subtitle_decoder_conf
        subtitle_decoder_choices,
    ]
    ...
    def add_task_arguments(cls, parser: argparse.ArgumentParser):
        ...
        group.add_argument(
            "--model_conf",
            action=NestedDictAction,
            default=get_default_kwargs(ESPnetSubtitleModelChained),
    ...
    @classmethod
    def build_model(cls, args: argparse.Namespace) -> ESPnetSubtitleModelChained:
        assert check_argument_types()
        ...
        if token_list is not None and args.model_conf.get('subtitle_ctc_weight', 0.0) > 0:
            subtitle_ctc = CTC(
                odim=vocab_size,
                encoder_output_sizse=encoder_output_size,
                **args.subtitle_ctc_conf,
            )
        else:
            subtitle_ctc = None

        if getattr(args, "subtitle_encoder", None) is not None:
            subtitle_input_size = encoder_output_size
            subtitle_encoder_class = subtitle_encoder_choices.get_class(args.subtitle_encoder)
            subtitle_encoder = subtitle_encoder_class(input_size=subtitle_input_size, **args.subtitle_encoder_conf)
            encoder_output_size = subtitle_encoder.output_size()
        else:
            subtitle_encoder = None

        if getattr(args, "subtitle_encoder", None) is not None:
            subtitle_decoder_class = subtitle_decoder_choices.get_class(
                args.subtitle_decoder
            )
            subtitle_decoder = subtitle_decoder_class(
                vocab_size=vocab_size,
                encoder_output_size=encoder_output_size,
                **args.subtitle_decoder_conf,
            )
        else:
            subtitle_decoder = None

        # 8. Build model
        model = ESPnetSubtitleModelChained(
            vocab_size=vocab_size,
