...
class ESPnetSubtitleModelChained(AbsESPnetModel):
    ...
    def forward(
        ...
        if self.interctc_weight != 0.0 and intermediate_outs is not None:
            ...
            for layer_idx, intermediate_out in intermediate_outs:
                ...
                loss_ic, cer_ic = self._calc_ctc_loss(
                    intermediate_out, encoder_out_lens, src_text, src_text_lengths, mask
                )
        ...
        # 3. Decoders
        if True:  #self.asr_weight > 0:
            mask_asr = src_text.sum(-1) > 0
            mask_subs = text.sum(-1) > 0

            if self.joint_decoder:
                loss_asr_att, acc_asr_att, cer_asr_att, \
                wer_asr_att, loss_subs_att, acc_subs_att = \
                    self._calc_att_loss_joint(encoder_out, encoder_out_lens,
    ...
    def _calc_att_loss(
            ...
            ys_pad_asr: torch.Tensor,  # src_text
            ys_pad_lens_asr: torch.Tensor,
            ys_pad_subs: torch.Tensor,  # text
            ys_pad_lens_subs: torch.Tensor,
    ...
    def _calc_att_loss_joint(
        ...
        max_len = max([ys_pad_asr.size(1), ys_pad_subs.size(1)])
        ys_pad_asr = F.pad(ys_pad_asr, [0, max_len - ys_pad_asr.size(1)],
                           'constant', self.ignore_id)
        ys_pad_subs = F.pad(ys_pad_subs, [0, max_len - ys_pad_subs.size(1)],
                            'constant', self.ignore_id)

        ys_pad = torch.where(mask_asr.unsqueeze(-1).expand(ys_pad_asr.shape),
                             ys_pad_asr, ys_pad_subs)
        ys_pad_lens = torch.where(mask_asr, ys_pad_lens_asr, ys_pad_lens_subs)

        if self.decoder_mlm:
            ys_in_pad, ys_out_pad = mask_uniform(
                ys_pad, self.src_mask_token, self.eos, self.ignore_id
            )
        ...
        # 2. ASR DECODER FORWARD
        decoder_out, decoder_out_lens = self.decoder(
            encoder_out, encoder_out_lens, ys_in_pad, ys_in_lens
        )

        if isinstance(decoder_out, tuple):
            #decoder_hidden = decoder_out[1]
            decoder_out = decoder_out[0]

        # 3. DECODER LOSS -- VERBATIM
        if mask_asr.sum() == 0:
            loss_asr_att, acc_asr_att, cer_asr_att, wer_asr_att = 0.0, None, None, None

        else:
            asr_decoder_out_m = decoder_out[mask_asr]
            ys_out_pad_asr_m = ys_out_pad[mask_asr]
            ys_pad_asr_m = ys_pad[mask_asr]
        ...
        # 4. DECODER LOSS -- SUBTITLES
        if mask_subs.sum() == 0:
            loss_subs_att, acc_subs_att, bleu_subs_att = 0.0, None, None
        else:
            subs_decoder_out_m = decoder_out[mask_subs]
            ys_out_pad_subs_m = ys_out_pad[mask_subs]
            ys_pad_subs_m = ys_pad[mask_subs]
        ...
        return loss_asr_att, acc_asr_att, cer_asr_att, wer_asr_att, loss_subs_att, acc_subs_att
    ...
    def _calc_mt_att_loss(
        ...
        if self.training or self.subtitle_error_calculator is None:
            ...
            bleu_att = self.subtitle_error_calculator(ys_hat.cpu(), ys_pad_m.cpu())
